			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<div class="col-md-6 col-sm-6">
						<p>No Faktur: <b><i>{{ $penjualan->main_id }}</i></b></p>
						<p>Foodhall: <b><i>{{ $penjualan->nama }}</i></b></p>
					</div>
					<div class="col-md-6 col-sm-6">
						<p>User: <b><i>{{ $penjualan->nama_lengkap }}</i></b></p>
						<p>Tanggal Kirim: <b><i>{{ date('j M Y', strtotime($penjualan->tgl_kirim)) }}</i></b></p>
					</div>
					<hr class="hr" />
					<div class="col-md-12">
...
<?php $i = 1;$total = 0;$total_qty = 0; ?>
								@foreach($detail as $det)
									<tr>
										...
										<td>{{ $det->harga }}</td>
										<td>{{ str_replace(',', '.', $det->kuantitas) * $det->harga }}</td>
									</tr>
<?php $i++;$total += str_replace(',', '.', $det->kuantitas) * $det->harga;$total_qty += str_replace(',', '.', $det->kuantitas) ?>
							...
							<tfoot>
								...
								<th><center><u>{{ $total_qty }}</u></center></th>
